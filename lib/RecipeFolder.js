...
const search_function = function() {
  let result_obj = {};
  let sql = `SELECT Entry,Sequence from lookup`;
  return new Promise( (resolve,reject) => {
    this.all(sql, (err,results) => {
      if (err) {
        ...
        results.forEach(row => {
          result_obj[row.Entry] = row.Sequence;
        });
        resolve(result_obj);
      }
    });
...
const load_databases = function(recipes) {
  const databases = {};
  let promises = [];
  recipes.forEach( recipe => {
    if (recipe.databases) {
      ...
      Object.keys(recipe.databases).forEach( db_alias => {
        databases[db_alias] = databases[db_alias] || new sqlite3.Database(recipe.databases[db_alias]);
        let query_func = search_function.bind(databases[db_alias]);
        promises.push(query_func().then( vals => recipe.environment[db_alias] = vals ));
      });
    }
  });
...
const RecipeFolder = function(input_path,options) {
  ...
  this.ready = this.findRecipes(this.path)
                   ...
                   .then((recipes) => load_databases(recipes) )
  ...
  recipes.forEach(function(recipe,idx) {
    ...
    recipe.sources.files.forEach(function(source) {
      ...
      if (! source_files[abs_path]) {
        source_files[abs_path] = { 'path' : path.join(recipe.base,source), 'recipes' : [], 'sheets' : [] };
      }
