...
const RecipeFolder = function(input_path,options) {
  ...
  this.recipe_streams = this.ready
                        .then(() => Promise.all(this.recipes.map((recipe) => this.needsUpdate(recipe,recipe.sources) )))
...
RecipeFolder.prototype.writeRecipe = function(stream,recipe) {
  ...
  let outstream = fs.createWriteStream(path.join((this.output || ''),recipe.filename));
