...
const helper_functions = {
  inject: function(arr,object) {
    return arr.map( element => { return { ...element, ...object}; });
  },
  join: function(arr,char) {
    if (arr.length > 0 && arr[0].value) {
      arr = arr.map( val => val.value );
    }
  ...
  split : function(string,pattern,clean=false,regexp=false) {
    return string.split(regexp ? new RegExp(pattern) : pattern).filter( x => clean ? x.length : x ).map( value => { return { value }; });
  },
